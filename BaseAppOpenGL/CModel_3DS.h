...
// by: Matthew Fairfax
//
// CModel_3DS.h: interface for the CModel_3DS class.
...
// Would have greatly bloated the model3DS class's code
...
public:
	...
	// I could have included a complex class but I wanted the model3DS class to stand alone
	struct Vector {
		float x;
		float y;
		float z;
	...
	char *modelname;		// The name of the model3DS
	char *path;				// The path of the model3DS
	int numObjects;			// Total number of objects in the model3DS
	int numMaterials;		// Total number of materials in the model3DS
	int totalVerts;			// Total number of vertices in the model3DS
	int totalFaces;			// Total number of faces in the model3DS
	bool shownormals;		// True: show the normals
	Material *Materials;	// The array of materials
	Object *Objects;		// The array of objects in the model3DS
	Vector pos;				// The position to move the model3DS to
	Vector rot;				// The angles to rotate the model3DS
	float scale;			// The size you want the model3DS scaled to
	bool lit;				// True: the model3DS is lit
	bool visible;			// True: the model3DS gets rendered
	void Load(char *name, 
		bool _isTileTextured = false, 
		float _fTileScaleU = 1.0f, 
		float _fTileScaleV = 1.0f);	// Loads a model3DS
	void Draw(bool textured = true);			// Draws the model3DS
	void DrawMultiTexture(CTexture* pTextures, int diffuseMapID, int normalMapID);
	FILE *bin3ds;			// The binary 3ds file
	CModel_3DS();			// Constructor
	virtual ~CModel_3DS();	// Destructor
...
private:
	...
	// Processes the model3DS's info
	void EditChunkProcessor(long length, long findex);

	// Processes the model3DS's materials
	...
	// Processes the faces of the model3DS and loads the faces
