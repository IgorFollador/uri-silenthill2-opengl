//////////////////////////////////////////////////////////////////////
//
// 3D Studio model3DS Class
// by: Matthew Fairfax
//
// CModel_3DS.cpp: implementation of the CModel_3DS class.
// This is a simple class for loading and viewing
// 3D Studio model3DS files (.3ds). It supports models
...
// ways for an artist to animate a 3D Studio model3DS and
...
// CModel_3DS m;
//
// m.Load("model3DS.3ds"); // Load the model3DS
// m.Draw();			// Renders the model3DS to the screen
//
// // If you want to show the model3DS's normals
// m.shownormals = true;
//
// // If the model3DS is not going to be lit then set the lit
// // variable to false. It defaults to true.
// m.lit = false;
//
// // You can disable the rendering of the model3DS
// m.visible = false;
// 
// // You can move and rotate the model3DS like this:
// m.rot.x = 90.0f;
// m.rot.y = 30.0f;
// m.rot.z = 0.0f;
//
...
CModel_3DS::CModel_3DS()
{
	...
	// The model3DS is lit by default
	lit = true;

	// The model3DS is visible by default
	visible = true;
...
CModel_3DS::~CModel_3DS()
{
	...
	for (int i = 0; i < numObjects; i++)
	{
		...
		delete[] Objects[i].MatFaces->subFaces;
...
void CModel_3DS::Load(char* name, bool _isTileTextured, float _fTileScaleU, float _fTileScaleV)
{
	this->isTileTextured = _isTileTextured;
	this->fTileScaleU = _fTileScaleU;
	this->fTileScaleV = _fTileScaleV;
	...
	{
		// Holds the name of the model3DS minus the path
		char* temp;
...
void CModel_3DS::Draw(bool textured)
{
	if (visible)
	{
		glPushMatrix();
		...
		for (int i = 0; i < numObjects; i++)
		{
			...
			if (shownormals)
			{
				...
				for (int k = 0; k < Objects[i].numVerts * 3; k += 3)
				{
					...
					// Disbale lighting if the model3DS is lit
					if (lit)
					...
					// If the model3DS is lit then renable lighting
					if (lit)
...
void CModel_3DS::DrawMultiTexture(CTexture* pTextures, int diffuseMapID, int normalMapID)
{
	glPushMatrix();
	...
	for (int i = 0; i < numObjects; i++)
	{
		...
		// Unidade de Textura 0 (Normal Map)
		glClientActiveTexture(GL_TEXTURE0);
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);
		glTexCoordPointer(2, GL_FLOAT, 0, Objects[i].TexCoords);
		glActiveTextureARB(GL_TEXTURE0);
		glEnable(GL_TEXTURE_2D);
		pTextures->ApplyTexture(normalMapID);
		...
		// Unidade de Textura 1 (Diffuse Map)
		glClientActiveTexture(GL_TEXTURE1);
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);
		glTexCoordPointer(2, GL_FLOAT, 0, Objects[i].TexCoords);
		glActiveTextureARB(GL_TEXTURE1);
		glEnable(GL_TEXTURE_2D);
		pTextures->ApplyTexture(diffuseMapID);
		glClientActiveTexture(GL_TEXTURE1);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
...
void CModel_3DS::CalculateNormals()
{
...
void CModel_3DS::MainChunkProcessor(long length, long findex)
{
	ChunkHeader h;
...
void CModel_3DS::EditChunkProcessor(long length, long findex)
{
	ChunkHeader h;
	...
	// Load the Objects (individual meshes in the whole model3DS)
	if (numObjects > 0)
	{
...
void CModel_3DS::MaterialChunkProcessor(long length, long findex, int matindex)
{
	ChunkHeader h;
...
void CModel_3DS::MaterialNameChunkProcessor(long length, long findex, int matindex)
{
	// move the file pointer to the beginning of the main
	// chunk's data findex + the size of the header
	fseek(bin3ds, findex, SEEK_SET);
...
void CModel_3DS::DiffuseColorChunkProcessor(long length, long findex, int matindex)
{
	ChunkHeader h;
...
void CModel_3DS::FloatColorChunkProcessor(long length, long findex, int matindex)
{
	float r;
	float g;
	float b;
...
void CModel_3DS::IntColorChunkProcessor(long length, long findex, int matindex)
{
	unsigned char r;
	unsigned char g;
	unsigned char b;
...
void CModel_3DS::TextureMapChunkProcessor(long length, long findex, int matindex)
{
	ChunkHeader h;
...
void CModel_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	char name[80];
...
void CModel_3DS::ObjectChunkProcessor(long length, long findex, int objindex)
{
	ChunkHeader h;
...
void CModel_3DS::TriangularMeshChunkProcessor(long length, long findex, int objindex)
{
	ChunkHeader h;
...
void CModel_3DS::VertexListChunkProcessor(long length, long findex, int objindex)
{
...
void CModel_3DS::TexCoordsChunkProcessor(long length, long findex, int objindex)
{
	...
	if (this->isTileTextured == true)
	{
		...
		for (int i = 0; i < numCoords * 2; i += 2)
		{
			...
			float u = Objects[objindex].TexCoords[i];
			float v = Objects[objindex].TexCoords[i + 1];

			Objects[objindex].TexCoords[i] = u * this->fTileScaleU;
			Objects[objindex].TexCoords[i + 1] = v * this->fTileScaleV;
		}
	}
	else
	{
...
void CModel_3DS::FacesDescriptionChunkProcessor(long length, long findex, int objindex)
{
	ChunkHeader h;
...
void CModel_3DS::FacesMaterialsListChunkProcessor(long length, long findex, int objindex, int subfacesindex)
{
